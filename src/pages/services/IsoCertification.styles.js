...
export const IsoCertificationContainer = styled.div`
    ...
    .iso-section{padding:48px 22px}
    .iso-container{max-width:1240px;margin:0 auto;display:grid;grid-template-columns:1fr 380px;gap:28px;align-items:start}
    .iso-main{background:#fff;padding:36px;border-radius:16px;box-shadow:0 12px 36px rgba(28,41,81,0.06);border:1px solid rgba(28,41,81,0.05)}
    .iso-main h1{font-size:clamp(1.75rem,3vw,2.75rem);margin-bottom:6px}
    ...
    .iso-sidebar{position:sticky;top:22px}
    ...
    @media(max-width:1024px){
        .iso-container{grid-template-columns:1fr}
        .iso-sidebar{position:static}
    }
`;
